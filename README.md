...
*Seven RX100M7 frames through the pipeline, one per column. Top to bottom: the camera JPEG (SDR
base), the RGB gain map the tool adds, the Ultra HDR result (renders brighter on an HDR display;
identical to the top row on SDR), and an SDR "preview" of that HDR rendition — a tone-mapped
fallback showing the recovered highlight detail for viewers without an HDR screen. The figure is
itself a single Ultra HDR JPEG, generated by [`tools/showcase`](tools/showcase).*
